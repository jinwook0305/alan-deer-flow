...
│   │           │   ├── chat_agent/    # Vanilla LLM chat agent (no tools, empty prompt)
...
**Chat Agent** (`packages/harness/deerflow/agents/chat_agent/agent.py`):
- Entry point: `make_chat_agent(config: RunnableConfig)` registered in `langgraph.json` alongside `lead_agent`
- Vanilla multi-turn LLM chat — no tools, no skills, no system prompt
- Middleware chain reduced to: `ThreadDataMiddleware` (always), `TokenUsageMiddleware` (if `token_usage.enabled`), `TitleMiddleware` (always), `MemoryMiddleware` (only when both `context.memory_enabled` and `app_config.memory.enabled` are true)
- Frontend selects this graph by setting `context.mode === "chat"` (drives `assistantId = "chat_agent"` in `useStream`)
- Used for plain conversational use cases that don't need the full harness
...
- `memory_enabled` - Chat-agent only: opt-in to MemoryMiddleware for vanilla chat (lead agent always queues memory when `app_config.memory.enabled`)
